...
func (c *PlatManager) getWechatPayHeader(headerFetcher func(string) string) (*WechatPayHeader, error) {
...
func (c *PlatManager) checkWechatPayHeader(ctx context.Context, args *WechatPayHeader) error {
...
// buildVerifyMessage 根据微信支付签名格式构造验签原文
func (c *PlatManager) buildVerifyMessage(ctx context.Context, headerArgs *WechatPayHeader, body []byte) string {
	// Suppressing warnings
	_ = ctx
...
// VerifyRequestMessage 验证 HTTP 消息
func (c *PlatManager) VerifyRequestMessage(ctx context.Context, headerFetcher func(string) string, body []byte) error {
	headerArgs, err := c.getWechatPayHeader(headerFetcher)
	if err != nil {
		return err
	}

	if err = c.checkWechatPayHeader(ctx, headerArgs); err != nil {
		return err
	}

	message := c.buildVerifyMessage(ctx, headerArgs, body)

	if err = c.LoadVerifier().Verify(ctx, headerArgs.Serial, message, headerArgs.Signature); err != nil {
		return fmt.Errorf(
