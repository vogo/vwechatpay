...
func (s *NativeClient) QueryOrderById(ctx context.Context, transactionId string) (*payments.Transaction, error) {
	// 构建请求参数
	...
	vlog.Infof("native query order | transaction_id: %s", transactionId)

	// 发送请求
	resp, result, err := s.native.QueryOrderById(ctx, req)
	if err != nil {
		vlog.Errorf("query order by id error | err: %v", err)
		return nil, err
	}

	vlog.Infof("native query order response | body: %s", vjson.EnsureMarshal(resp))
...
// outTradeNo: 商户订单号
func (s *NativeClient) QueryOrderByOutTradeNo(ctx context.Context, outTradeNo string) (*payments.Transaction, error) {
	// 构建请求参数
	...
	vlog.Infof("native query request | out_trade_no: %s", outTradeNo)

	// 发送请求
	resp, result, err := s.native.QueryOrderByOutTradeNo(ctx, req)
	if err != nil {
